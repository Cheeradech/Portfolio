...
import openHouseImage from '../assets/openh.jpg';

const Activity = React.memo(() => {
    const { lang } = useLanguage();
    const t = translations[lang].activity;
    const [isImageOpen, setIsImageOpen] = useState(false);
    const [isOpenHouseImageOpen, setIsOpenHouseImageOpen] = useState(false);

    useEffect(() => {
        if (!isImageOpen && !isOpenHouseImageOpen) return;
    ...
    }, [isImageOpen, isOpenHouseImageOpen]);
    ...
    return (
        <section id="activity" className="py-24 px-4 sm:px-6 lg:px-12 relative z-10 scroll-mt-24">
            <div className="max-w-6xl mx-auto">
                ...
                {/* BU Open House 2026 Card */}
                <motion.div
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1], delay: 0.15 }}
                    className="relative bg-[#0a0a0a]/60 border border-white/5 rounded-2xl p-6 sm:p-8 lg:p-10 backdrop-blur-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] overflow-hidden group hover:bg-[#111111]/80 hover:border-white/10 transition-all duration-500 mt-6"
                >
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center relative z-10">
                        ...
                        <div className="lg:col-span-5">
                            <div
                                onClick={() => setIsOpenHouseImageOpen(true)}
                                className="relative rounded-xl overflow-hidden border border-white/5 group/img cursor-pointer bg-[#050505] aspect-[4/3] flex items-center justify-center"
                            >
                                <img
                                    src={openHouseImage}
                                    alt="BU Open House 2026"
                                    className="w-full h-full object-contain opacity-90 transition-all duration-700 group-hover/img:scale-[1.02] group-hover/img:opacity-100"
                                />

                                {/* Event Badge Overlay */}
                                <div className="absolute top-4 left-4 px-3 py-1.5 bg-black/60 backdrop-blur-md border border-white/10 rounded-md text-[11px] font-medium text-slate-200 flex items-center gap-1.5 uppercase tracking-wide shadow-sm">
                                    <MapPin strokeWidth={1.5} className="w-3.5 h-3.5 text-slate-300" />
                                    <span>BU Open House 2026</span>
                                </div>
                        ...
                        <div className="lg:col-span-7 flex flex-col justify-center space-y-6">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400 font-light">
                                <div className="flex items-center gap-2">
                                    <UserCheck strokeWidth={1.5} className="w-4 h-4 text-slate-500" />
                                    <span>{t.roleLabel}: <span className="text-slate-300 font-normal">{t.openHouseRole}</span></span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <MapPin strokeWidth={1.5} className="w-4 h-4 text-slate-500" />
                                    <span>Bangkok University</span>
                            ...
                            {/* Feature Chips */}
                            <div className="flex flex-wrap gap-2 pt-2">
                                {(t.openHouseChips || ['Event Staff', 'Project Showcase', 'Industry-Driven Innovation']).map((chip, idx) => (
                                    <span
                                        key={idx}
                                        className="px-3 py-1.5 rounded-md text-[10px] sm:text-[11px] uppercase tracking-widest font-medium bg-white/5 text-slate-400 border border-transparent hover:border-white/10 hover:text-slate-300 transition-colors"
                                    >
            ...
            {/* Lightbox Modal — IT Empowering Day */}
            ...
            {/* Lightbox Modal — BU Open House 2026 */}
            {typeof document !== 'undefined' && createPortal(
                <AnimatePresence>
                    {isOpenHouseImageOpen && (
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            onClick={() => setIsOpenHouseImageOpen(false)}
                            className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-4 sm:p-8 cursor-pointer"
                        >
                            <div className="relative max-w-5xl max-h-[90vh] w-full flex items-center justify-center">
                                <button
                                ...
                                <img
                                    src={openHouseImage}
                                    alt="BU Open House 2026 Full View"
                                    className="max-w-full max-h-[85vh] object-contain rounded-xl border border-white/5 shadow-2xl"
                                />
                            </div>
                        </motion.div>
